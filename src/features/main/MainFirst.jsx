                    ...
                    <Main.FirstMainSliderTitle
                      children={
                        <>
                          ...
                          <div
                            className="title"
                            children={personalEx.exhibitionTitle}
                          />
                        </>
                      }
                    />
                    <Main.FirstMainSliderImg src={personalEx.postImage} />
                    <Main.FirstMainSliderDesc
                      children={personalEx.exhibitionDesc}
                    />
                    <Main.FirstMainSliderBtn
                      onClick={() => navigatehandle(personalEx.detailRouter)}
                      children="자세히 보러가기"
                    />
                    <Main.FirstMainSliderInfo>
                      <Main.FirstMainSliderInfoText
                        children={
                          <>
                            <p className="title" children="기간" />
                            <p
                              className="content contentdate"
                              children={`${editTimehandle(
                                personalEx.startDate
                              )} - ${editTimehandle(personalEx.endDate)}`}
                            />
                          </>
                        }
                      />
                        ...
                        children={
                          <>
                            ...
                            <p
                              className="content"
                              children={
                                personalEx.author.length === 1
                                  ? personalEx.author.map(author => `${author}`)
                                  : personalEx.author.length < 5
                                  ? personalEx.author.map((author, index) =>
                                      personalEx.author.length === index + 1
                                        ? `${author}`
                                        : `${author}, `
                                    )
                                  : `${personalEx.author[0]}외 ${
                                      personalEx.author.length - 1
                                    }명`
                              }
                            />
          ...
          <Main.FirstSubSlider
            children={
              <Slider {...secondSliderSettings}>
                {editLists &&
                  editLists.map(personalEx => (
                    <Main.FirstSubSliderWrap
                      key={personalEx.exhibitionId}
                      children={
                        <Main.FirstSubSliderImg
                          src={personalEx.postImage}
                          alt="개인전 이미지"
                        />
                      }
                    />
